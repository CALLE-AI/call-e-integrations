...
function tempCacheRoot() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "calle-broker-example-ts-"));
}
...
test("broker example creates login, exchanges token, calls tool, reads resource, then reuses cache", async (t) => {
...
test("broker example resumes pending login and handles no resources", async (t) => {
...
test("broker example refreshes expired cache and reports remote 401 as auth_required", async (t) => {
...
test("broker example clears stale cached token rejected by MCP server", async (t) => {
  const fake = await startFakeServer();
  t.after(() => fake.close());
  const cacheRoot = tempCacheRoot();
...
test("broker example discards hostile cached login data before output or broker requests", async (t) => {
  const fake = await startFakeServer();
  t.after(() => fake.close());
  const cacheRoot = tempCacheRoot();
...
test("broker example rejects hostile broker responses before caching or output", async (t) => {
...
test("broker example encodes opaque session IDs at request paths", async (t) => {
