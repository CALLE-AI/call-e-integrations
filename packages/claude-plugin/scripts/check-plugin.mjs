...
function assertCliGuidance({ source, filePath, failures }) {
  ...
  assert(!/(?:^|[^\w/])\/mcp\b/u.test(source), failures, `${displayPath(filePath)} must not direct users to /mcp for CALL-E authorization.`);
...
function checkDocs({ packageRoot, repoRoot, failures }) {
  ...
  if (fs.existsSync(installDoc)) {
    ...
    assert(source.includes('["auth", "login"]'), failures, "Claude install doc must document authorization through the verified CLI entry point.");
